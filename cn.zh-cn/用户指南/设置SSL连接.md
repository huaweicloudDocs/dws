...
4.  在“SSL连接“区域中，单击“服务器端是否强制使用SSL连接“的设置开关进行设置，建议开启。
    ...
    ![](figures/zh-cn_image_0106894485.png)：为开启，表示服务器端强制要求SSL连接。

    ![](figures/zh-cn_image_0106894679.jpg)：为关闭，表示服务器端对是否通过SSL连接不作强制要求，默认为关闭。
    ...
    **图 1**  SSL连接<a name="fig168181335124718"></a>  
    ...
    >-   如果使用DWS提供的JDBC驱动，支持的SSL协议有SSLv3、TLSv1、TLSv1.1、TLSv1.2。客户端与数据库之间实际使用何种SSL协议，依赖客户端使用的JDK版本，一般JDK支持多个SSL协议。  
