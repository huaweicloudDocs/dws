...
4.  选择“主机规格“。
    ...
    **表 2**  主机规格参数说明
    ...
    <a name="ul139998019510"></a><a name="ul139998019510"></a><ul id="ul139998019510"><li>标准企业级数仓：提供大数据量、以批处理查询为主的数据查询能力。</li><li>超高并发型数仓：提供小数据量、高并发、低延时为主的交互式查询能力。</li></ul>
    </td>
    ...
    **表 3**  规格说明
    ...
    <td class="cellrowborder" valign="top" width="16.37%" headers="mcps1.2.6.1.4 "><p id="p197081535194917"><a name="p197081535194917"></a><a name="p197081535194917"></a>16.75 TB</p>
    </td>
...
7.  高级配置，可选择如下选项：
    -   **默认配置**：表示以下几项高级配置使用系统默认的配置。
        -   参数组：默认将系统默认的数据库参数组“Default-Parameter-Group-DWS“与集群相关联。
        -   企业项目：表示集群所属的企业项目。已开通企业管理服务的用户才可以配置该参数。默认值为default。
        -   标签：默认未给集群添加标签。
        -   加密数据库：默认为不启用数据库加密。
    ...
    -   **自定义**：选择该选项时，请自定义设置以下几项高级配置。
        -   **参数组**
            ...
            参数组是一组数据库参数的集合。在下拉列表中选择一个参数组与集群相关联。可以选择默认的参数组“Default-Parameter-Group-DWS“或者自定义创建的参数组。

            有关参数组的更多信息，请参见[管理参数组](管理参数组.md)。

        -   **企业项目**

            为集群选择所属的企业项目。已开通企业管理服务的用户才可以配置该参数。
        ...
        -   **加密数据库**
            ...
            ![](figures/icon_dws_on.png)![](figures/wwx437827-中软基础平台部-DataSight-image-2684cf81-d351-4504-ae1a-2fb70826e877.png)表示启用数据库加密。每个区域的每个项目首次启用数据库加密时，系统会弹出一个“创建委托”的对话框，单击“是“创建委托以授权DWS访问KMS并创建一个KMS基础版密钥（该密钥不能用于DWS数据库加密），若单击“否“将不会启用加密功能。然后在“密钥名称“的下拉列表中选择已创建的KMS专业版密钥。如果没有专业版密钥，可以登录KMS服务进行创建，详细操作请参见[《数据加密服务用户指南》](https://support.huaweicloud.com/dew/index.html)。
